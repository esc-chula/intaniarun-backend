...
const user = {
    ...
    citizenId: joi.string().custom((value, helper) => {
        if (!validateThaiID(value))
            return helper.message({ custom: 'Invalid citizen ID' });
        return value;
    }).required(),
    nationality: joi.string().required(),
    shirtSize: joi.string().valid('XS', 'S', 'M', 'L', 'XL', 'XXL', 'XXL').required(),
...
export const userUpdateSchema = joi.object(user).fork(Object.keys(user), (schema) => schema.optional())